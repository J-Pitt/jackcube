...
import { leaveRoom, leaveRoomBeacon, startGame } from '@/lib/roomApi'
...
export default function LobbyPage() {
  ...
  const { room, error: pollError, loading } = useRoomPoll(roomId)

  useEffect(() => {
    if (room && session && !session.playerId && room.players?.length) {
      const byName = session.playerName
        ? room.players.find((p) => p.name === session.playerName)
        : null
      if (byName) {
        setSession((s) => ({ ...s, playerId: byName.id }))
      }
    }
  }, [room, session])
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          >
            ...
            {isHost && (
              <button
                type="button"
                onClick={handleStart}
                disabled={startLoading || playerNames.length < 2}
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
            <LobbyVideo
              roomId={roomId}
              myPlayerId={session?.playerId}
              myPlayerName={myPlayer?.name || session?.playerName || 'You'}
              players={playerNames}
            />
          </motion.div>
        </div>
