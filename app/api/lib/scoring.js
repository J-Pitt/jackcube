...
function scoreRound(players, roundScores) {
  ...
  const results = ranked.map((p, index) => {
    ...
    const points = (PLACEMENT_POINTS[index] ?? PARTICIPATION) + PARTICIPATION
    return {
      playerId: p.id,
      name: p.name,
...
function checkVictory(players, targetScore) {
  const winner = players.find((p) => (Number(p.score) || 0) >= targetScore)
  return winner || null
}

export { scoreRound, checkVictory }
