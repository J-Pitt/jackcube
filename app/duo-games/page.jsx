...
import { DUEL_GAMES } from '@/lib/games/registry'
import { getAccent } from '@/components/game/GameUI'

function DuoGamesContent() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
        <h1 className="font-display text-4xl font-extrabold text-white">Duo games</h1>
        <p className="mt-2 text-white/50">
          {modeLabel
            ? `${modeLabel} head-to-head — exactly 2 players.`
            : 'Head-to-head party games for two — one TV, two phones. Inspired by classics like Floor It and 1v1 Pictionary.'}
        </p>
      </motion.div>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        {DUEL_GAMES.map((g, i) => {
          ...
          const playerLabel = g.minPlayers === g.maxPlayers ? `${g.minPlayers} players` : `${g.minPlayers}–${g.maxPlayers} players`
          return (
            <motion.div
              key={g.id}
            ...
            >
              ...
              <p className="mt-3 text-xs font-semibold uppercase tracking-widest text-white/30">
                {playerLabel} · {g.maxRounds} rounds
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                {mode ? (
                  ...
                  >
                    Host {modeLabel.toLowerCase()} duel
                  </Link>
                ) : (
                  <>
                    <Link
...
export default function DuoGamesPage() {
  return (
